...
This function contains nearly all of the logic you'll need in order 
to implement Quick Sort in the next exercise.

The pivot  function is responsible for taking an array, 
setting the pivot value, and mutating the array so that all values less than the pivot 
wind up to the left of it, and all values greater than the pivot wind up to the right of it. 
...
an an optional comparator, but also an optional start and end index. 
...
function pivot(arr, comparator, start=0, end = arr.length - 1) {
  ...
  // set the pivot value as the first element
  
  let pivotValue = arr[start];
  let pivotIndex = start;
  
  // Default comparator function for numbers (ascending numbers)
  ...
  for(let i = start + 1; i <= end; i++ ) {
    //   if the current element less than the pivot value swap It.
...
console.log(arr1); // The array is rearranged such that elements less than the pivot are on the left, and elements greater are on the right
