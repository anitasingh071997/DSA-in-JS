...
Assign the first element to be the smallest value (this is called
the minimum). It does not matter right now if this actually the smallest value in the array.
...
swap the two values. You will now see that the beginning of the array is in the correct order
...
function selectionSort(arr, comparator) {
    // Default comparator function for ascending numeric order
    ...
    for (let i = 0; i < arr.length - 1; i++) {
      ...
      // If minIndex is not the current position, swap the elements
      if (minIndex !== i) {
